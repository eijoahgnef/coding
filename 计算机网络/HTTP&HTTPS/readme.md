...
- 浏览器发送它所支持的加密套件列表和一个随机数 client-random，这里的 __加密套件是指加密的方法__，加密套件列表就是指浏览器能支持多少种加密方法列表。
- 服务器会从加密套件列表中选取一个加密套件，然后还会生成一个随机数 service-random，并将 service-random 和加密套件列表返回给浏览器。
- 最后浏览器和服务器分别返回确认消息。
这样浏览器和服务端都有相同的client-random和service-random了，然后它们再使用相同的方法将这两个随机数混合起来生成一个密钥 master secret，有了这个密钥和加密套件之后，双发就可以进行数据的加密传输了。

存在的问题：由于传输client-random和service-random的过程中是明文的，这意味着黑客可以获取到协商的加密套件和双方的随机数，由于利用随机数合成秘钥的算法是公开的，所以黑客拿到随机数之后，也可以合成密钥，这样数据依然可以破解。

## 第二个版本 使用非对称加密
非对称加密算法有A、B两把密钥，如果你用A密钥来加密，那么只能使用B密钥来解密；反过来，如果你要B密钥来加密，那么只能用A密钥来解密。

在HTTPS中，服务器会将其中有一个密钥通过明文的形式发送给浏览器，我们把这个密钥称为`公钥`，服务器自己留下的那个密钥称为`私钥`。
...
存在的问题：
- 对称加密的效率太低。这会严重影响到加密数据的速度，进而影响到用户的打开页面的速度。
- 无法保证服务器发送给浏览器的数据安全。因为公钥是能获取到的，用公钥可以用来解密服务器用私钥加密的数据。

## 第三个版本 对称加密和非对称加密搭配使用
基于以上两点，最终选择一个更加完美的方案，那就是 __在数据传输阶段依然使用对称加密，但是对称加密的密钥采用非对称加密来传输__。
...
相对于第三版的HTTPS协议，主要有两点改变：
- 服务器没有直接返回公钥给浏览器，而是返回来数字证书，而公钥正式包含在数字证书中的。
- 在浏览器端多了一个证书验证的操作，验证了证书，才继续后续流程。

申请CA证书的流程：
- 首先需要准备一套私钥和公钥，私钥留着自己使用
- 然后向CA机构提交公钥、公司、站点等信息并等待认证，这个认证过程可能是收费的
- CA通过线上、线下等多种渠道来验证该公司所提供信息的真实性，如公司是否存在、企业是否合法、域名是否归属该企业等
- 如信息审核通过，CA会向该公司签发认证的数字证书，包含了该服务器的公钥。组织信息、CA的信息、有效时间、证书序列号等，这些信息都是明文的，同时包含一个CA生成的签名。